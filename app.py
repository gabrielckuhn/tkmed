...
if 'page' not in st.session_state:
    st.session_state.page = 'home' # 'home' ou 'report'
...
def reset_app():
    """Limpa o estado para fazer uma nova busca"""
...
# --- CSS GERAL ---
# CSS que será injetado no HTML do componente (O mesmo de antes)
report_css = f"""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap');
    body {{ font-family: 'Roboto', Arial, sans-serif; margin: 0; padding: 20px; background-color: {COLOR_BG}; color: {COLOR_DARK}; font-size: 10pt; display: block; }}
    #container {{ width: 793px; margin: 0 auto; background-color: {COLOR_BG}; padding: 0; box-sizing: border-box; }}
    ...
    .font-p {{ font-size: 12px; }} .font-m {{ font-size: 14px; }} .font-g {{ font-size: 16px; }} .font-bold {{ font-weight: bold; }}
    ...
    .corpo>div:nth-child(1) {{ text-align: right; }} .corpo>div:nth-child(1)>div:nth-child(2) {{ margin-top: 10px; }} .corpo>div:nth-child(1)>div:nth-child(3) {{ margin-top: 30px; }} .corpo>div:nth-child(1)>div:nth-child(4) {{ margin-top: 63px; }}
    .corpo>div:nth-child(2) {{ text-align: center; margin-top: 66px; }} .corpo>div:nth-child(3)>div:nth-child(2) {{ margin-top: 10px; }} .corpo>div:nth-child(3)>div:nth-child(3) {{ margin-top: 140px; }}
    ...
    .cel-cinza {{ background-color: {COLOR_LIGHT}; color: {COLOR_DARK}; }} .cel-verde {{ background-color: {COLOR_PRIMARY}; }}
    .cel-header {{ color: {COLOR_DARK}; font-size: 18px; font-weight: bold; text-align: center; padding: 4px; }} .cel-verde.cel-header {{ color: #FFF; }}
    ...
    .header {{ margin-bottom: 10px; }} .logo-cel img {{ max-height: 80px; }} .user-cel {{ text-align: right; color: {COLOR_DARK}; }} .user-cel .nome {{ font-size: 12pt; font-weight: bold; }}
    .dados-paciente {{ background-color: {COLOR_BG}; padding: 10px 20px; border-bottom: 1px solid {COLOR_LIGHT}; }} .dados-paciente .label {{ font-weight: bold; color: {COLOR_DARK}; display: inline; }}
    ...
    .grafico-valores {{ display: grid; grid-template-columns: repeat(11, 1fr); grid-gap: 2px; }} .grafico-valores>div {{ text-align: center; }}
    .grafico-valores>div>div {{ background-color: {COLOR_DARK}; width: 2px; height: 5px; display: block; margin-left: calc(50% - 1px); }}
    .barra-grafico-container {{ font-size: 20px; font-weight: bold; margin-top: 2px; }} .barra-grafico {{ background-color: {COLOR_DARK}; height: 15px; display: inline-block; margin-right: 10px; max-width: calc(100% - 70px); }}
    .cel-grafico-p {{ border-top: {COLOR_DARK} 1px solid; border-left: {COLOR_DARK} 1px solid; }} .barra-grafico-p-container {{ font-size: 12px; font-weight: bold; margin-top: 3px; }} .barra-grafico-p {{ background-color: {COLOR_DARK}; height: 7px; display: inline-block; margin-right: 10px; }}
    #charts {{ width: 100%; min-width: 750px; border-collapse: separate; border-spacing: 5px; }}
    #charts tr td:nth-child(1) {{ width: 110px; background-color: {COLOR_PRIMARY}; font-weight: bold; color: #FFF; padding: 10px; min-height: 32px; }}
    .chartPlaceholder {{ position: absolute; width: calc(81.5% + 30px); height: 56px; overflow: hidden; left: calc(8.33% - 5px); }}
    .graficos-tr {{ height: 38px; }} .valor-label {{ border-right: dashed 2px {COLOR_LIGHT}; height: 56px; font-size: 11px; padding: 0 8px; z-index: 1000; color: {COLOR_DARK}; }}
    .grafico-label {{ text-align: center; font-weight: bold; font-size: 12px; color: {COLOR_DARK}; background-color: {COLOR_LIGHT}; padding: 3px; }}
    .datas {{ grid-gap: 3px; height: 20px; }} .quebra-de-pagina {{ page-break-before: always; }}
    @media (max-width: 700px) {{ #container {{ width: 100%; }} .grid-container-normalidades {{ min-width: 100%; }} .barra-corpos {{ display: none; }} .grid-container-3c-paciente {{ grid-template-columns: 1fr; }} }}
</style>
"""
...
if st.session_state.page == 'home':
    # --- PÁGINA 1: INPUT ---
    # Centralizar o input visualmente
    col_spacer_l, col_content, col_spacer_r = st.columns([1, 2, 1])
    
    with col_content:
        st.markdown("### Gerador de Relatórios")
        url_input = st.text_input(
            "Cole o link do relatório ou o ID:", 
            placeholder="Ex: https://...#CODIGO-DO-RELATORIO"
        )
        
        # Se houver input (usuário apertou Enter)
        if url_input:
            ...
            if report_id:
                with st.spinner('Buscando dados...'):
                    ...
                    if data:
                        ...
                        st.session_state.page = 'report' # Muda para a página de relatório
                        st.rerun() # Recarrega o app imediatamente
                    else:
                        st.error("Relatório não encontrado ou erro de conexão.")
            else:
                st.warning("Link ou ID inválido.")

elif st.session_state.page == 'report':
    # --- PÁGINA 2: RELATÓRIO ---
    
    # Botões de Controle (Nova Pesquisa e Download) ficam no topo
    col_back, col_download, col_rest = st.columns([1, 1, 4])
    ...
    if data:
        ...
        <script>
            ...
            // ... (Resto das variáveis e funções JS mantidas, apenas estrutura aqui)

            document.addEventListener("DOMContentLoaded", function () {{
                // Inicialização dos dados e gráficos
                aplicarTraducoes();
                popularDadosUsuario(apiData.user);
                popularDadosPaciente(apiData.paciente);
                const ultimaAvaliacao = apiData.avaliacoes[apiData.avaliacoes.length - 1];
                document.getElementById("data").innerText = formatarData(ultimaAvaliacao.data || new Date());
                popularNormalidades(ultimaAvaliacao, apiData.normalidades);
                popularDadosMembros(ultimaAvaliacao.dadosMembros, ultimaAvaliacao);
                popularDadosAdicionais(ultimaAvaliacao);
                criaLabelGrafico(apiData.avaliacoes);
                criarGraficos(apiData);
            ...
            // --- FUNÇÕES DE LÓGICA JS (Compactadas para não ocupar espaço, são as mesmas do seu código anterior) ---
            ...
            function criarGrafico(avaliacoesData, prop, label, translationKey, utilizarFormaDecimalPadrao = true) {{
                const valores = avaliacoesData.map(avaliacao => obterValor(avaliacao, prop)); while (valores.length < 6) {{ valores.push(null); }}
                ...
                valores.forEach(v => {{ const valorLabel = document.createElement("div"); valorLabel.className = "valor-label"; if (v) valorLabel.innerText = utilizarFormaDecimalPadrao ? formatarNumeroDecimalBrasileiro(v) : formatarNumeroBrasileiro(v); valoresLabel.appendChild(valorLabel); }}); td2.appendChild(valoresLabel);
            ...
            function popularDadosMembros(dadosMembro, avaliacao) {{
                document.getElementById("mm-bd-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[0].composicaoCorporal.ffm) + "kg"; document.getElementById("mm-bd-p").innerText = formatarNumeroBrasileiro(dadosMembro[0].composicaoCorporal.ffm / avaliacao.peso * 100) + "%";
                document.getElementById("mm-be-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[1].composicaoCorporal.ffm) + "kg"; document.getElementById("mm-be-p").innerText = formatarNumeroBrasileiro(dadosMembro[1].composicaoCorporal.ffm / avaliacao.peso * 100) + "%";
                document.getElementById("mm-t-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[2].composicaoCorporal.ffm) + "kg"; document.getElementById("mm-t-p").innerText = formatarNumeroBrasileiro(dadosMembro[2].composicaoCorporal.ffm / avaliacao.peso * 100) + "%";
                document.getElementById("mm-pd-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[3].composicaoCorporal.ffm) + "kg"; document.getElementById("mm-pd-p").innerText = formatarNumeroBrasileiro(dadosMembro[3].composicaoCorporal.ffm / avaliacao.peso * 100) + "%";
                document.getElementById("mm-pe-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[4].composicaoCorporal.ffm) + "kg"; document.getElementById("mm-pe-p").innerText = formatarNumeroBrasileiro(dadosMembro[4].composicaoCorporal.ffm / avaliacao.peso * 100) + "%";
                document.getElementById("mm-c-k").innerText = formatarNumeroDecimalBrasileiro(avaliacao.dadosCorpo.ffm) + "kg"; document.getElementById("mm-c-p").innerText = formatarNumeroBrasileiro(avaliacao.dadosCorpo.ffm / avaliacao.peso * 100) + "%";
                document.getElementById("g-bd-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[0].composicaoCorporal.fm) + "kg"; document.getElementById("g-bd-p").innerText = formatarNumeroBrasileiro(dadosMembro[0].composicaoCorporal.fm / avaliacao.peso * 100) + "%";
                document.getElementById("g-be-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[1].composicaoCorporal.fm) + "kg"; document.getElementById("g-be-p").innerText = formatarNumeroBrasileiro(dadosMembro[1].composicaoCorporal.fm / avaliacao.peso * 100) + "%";
                document.getElementById("g-t-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[2].composicaoCorporal.fm) + "kg"; document.getElementById("g-t-p").innerText = formatarNumeroBrasileiro(dadosMembro[2].composicaoCorporal.fm / avaliacao.peso * 100) + "%";
                document.getElementById("g-pd-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[3].composicaoCorporal.fm) + "kg"; document.getElementById("g-pd-p").innerText = formatarNumeroBrasileiro(dadosMembro[3].composicaoCorporal.fm / avaliacao.peso * 100) + "%";
                document.getElementById("g-pe-k").innerText = formatarNumeroDecimalBrasileiro(dadosMembro[4].composicaoCorporal.fm) + "kg"; document.getElementById("g-pe-p").innerText = formatarNumeroBrasileiro(dadosMembro[4].composicaoCorporal.fm / avaliacao.peso * 100) + "%";
                document.getElementById("g-c-k").innerText = formatarNumeroDecimalBrasileiro(avaliacao.dadosCorpo.fm) + "kg"; document.getElementById("g-c-p").innerText = formatarNumeroBrasileiro(avaliacao.dadosCorpo.fm / avaliacao.peso * 100) + "%";
            }}
